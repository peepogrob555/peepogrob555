...
echo -e "\n${B}${C}  VPS Setup — VLESS REALITY | TCP RAW | port 443 | SNI speedtest.net | Firefox fingerprint${N}"
echo -e "  Interface: ${Y}$ETH${N} | IP: ${Y}$PUBIP${N}\n"
echo -e "  Target: 1Gbps/conn | RTT 20ms | TLS 1.3 | AIS bypass\n"
...
if ufw status | grep -q "Status: active"; then
    for p in "${PORTS[@]}"; do
        ufw allow "$p"/tcp comment "3xui-reality" >/dev/null 2>&1
        ok "UFW: $p/tcp"
    done
...
sec "STEP 5 — SYSCTL (VLESS Reality | TCP RAW | TLS 1.3 | 1Gbps | RTT 20ms)"

modprobe tcp_bbr 2>/dev/null && ok "tcp_bbr loaded" || inf "tcp_bbr built-in"

cat > /etc/sysctl.d/99-ais-reality.conf << 'EOF'
...
sec "STEP 8 — VLESS REALITY SOCKOPT PATCHER (TCP RAW | TLS 1.3 | port 443)"

PATCHER=/usr/local/bin/xui-reality-patch.py
cat > "$PATCHER" << 'PYEOF'
#!/usr/bin/env python3
import json, sys, os
...
def patch(path):
    ...
    for ib in inbounds:
        if not isinstance(ib, dict):
            continue
        if ib.get("protocol") != TARGET_PROTOCOL:
            continue
        ss = ib.get("streamSettings", {})
        if ss.get("security") != TARGET_SECURITY:
            continue
        ...
        patched_count += 1
    if patched_count == 0:
        print(f"  [{path}] no VLESS Reality inbounds found yet — skipping sockopt patch")
        return 0
    try:
        ...
        return 0
    print(f"  [{path}] patched {changed} fields across {patched_count} VLESS Reality inbound(s)")
    return changed
...
print("  no xray config found — patcher will run on every x-ui start")
sys.exit(0)
PYEOF
chmod +x "$PATCHER"
ok "xui-reality-patch.py created"

mkdir -p /etc/systemd/system/x-ui.service.d/
cat > /etc/systemd/system/x-ui.service.d/reality-patch.conf << 'UNITEOF'
[Service]
ExecStartPost=/bin/bash -c 'for i in $(seq 15); do sleep 2 && python3 /usr/local/bin/xui-reality-patch.py && break; done'
UNITEOF
ok "reality sockopt patcher hooked to x-ui"

inf "running reality-patch now..."
python3 "$PATCHER"
...
echo ""
echo -e "${G}${B}╔══════════════════════════════════════════╗${N}"
echo -e "${G}${B}║  DONE — VLESS Reality Setup complete!    ║${N}"
echo -e "${G}${B}╚══════════════════════════════════════════╝${N}"
echo ""
echo -e "${Y}  ▸ 3x-ui panel      : http://${PUBIP}:${PANEL_PORT}${N}"
echo -e "${Y}  ▸ protocol         : VLESS${N}"
echo -e "${Y}  ▸ transmission     : TCP (RAW)${N}"
echo -e "${Y}  ▸ security         : Reality${N}"
echo -e "${Y}  ▸ port             : 443${N}"
echo -e "${Y}  ▸ SNI              : speedtest.net${N}"
echo -e "${Y}  ▸ fingerprint      : firefox${N}"
echo -e "${Y}  ▸ TCP Max Seg      : 1380 (TLS 1.3 overhead compensated)${N}"
echo -e "${Y}  ▸ tcpUserTimeout   : 8000ms (TLS handshake safe)${N}"
echo -e "${Y}  ▸ tcp_mtu_probing  : 0 (ปิด — Reality จัดการ MTU เอง)${N}"
echo -e "${Y}  ▸ CAKE             : 1gbit fair queue ทุก flow${N}"
echo -e "${Y}  ▸ sockopt patcher  : runs auto บน VLESS Reality inbound เท่านั้น${N}"
echo -e "${Y}  ▸ VMESS WS         : ยังทำงานปกติ ไม่ถูกแตะ${N}"
echo -e "${Y}  ▸ reboot           : sudo reboot${N}"
echo -e "${Y}  ▸ nofile 1000000   : มีผลหลัง reboot${N}"
echo ""
echo -e "${C}  ── ขั้นตอนสร้าง inbound ใน panel ──${N}"
echo -e "${Y}  1. Inbounds → Add Inbound${N}"
echo -e "${Y}  2. Protocol: vless | Port: 443${N}"
echo -e "${Y}  3. Transmission: TCP | Security: Reality${N}"
echo -e "${Y}  4. SNI: speedtest.net | Fingerprint: firefox${N}"
echo -e "${Y}  5. กด Get New Cert (สร้าง Reality keypair)${N}"
echo -e "${Y}  6. Save → รอ patcher ทำงานอัตโนมัติ (~30 วินาที)${N}"
echo ""
