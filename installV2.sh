...
echo ""
echo -e "${BLD}${CYN}╔══════════════════════════════════════════════════════════╗${RST}"
echo -e "${BLD}${CYN}║  AIS กันรั่ว — VMESS WS v5.0 ULTRA SUPER MEGA ULL       ║${RST}"
echo -e "${BLD}${CYN}║  1vCPU · 2GB RAM · KVM virtio_net · RTT~30ms AIS LTE    ║${RST}"
echo -e "${BLD}${CYN}║  ปิงต่ำสุด · jitter ต่ำสุด · เก็บทุกรายละเอียดทุกนิ้ว   ║${RST}"
echo -e "${BLD}${CYN}║  NIC: ${NIC}$(printf '%*s' $((51-${#NIC})) '')║${RST}"
echo -e "${BLD}${CYN}╚══════════════════════════════════════════════════════════╝${RST}"
echo ""
info "BDP @150Mbps 30ms : ${BDP} bytes (563KB/conn)"
info "notsent_lowat     : ${NOTSENT_LOWAT} bytes (4KB)"
info "limit_output      : ${LIMIT_OUTPUT} bytes (1xBDP)"
...
_step7() {
  ...
  ethtool -C "$NIC" rx-usecs 0 tx-usecs 0 adaptive-rx off adaptive-tx off \
    rx-frames 1 tx-frames 1 2>/dev/null \
  ...
  ip link set "$NIC" txqueuelen 128
...
ethtool -C "\${NIC}" rx-usecs 0 tx-usecs 0 adaptive-rx off adaptive-tx off rx-frames 1 tx-frames 1 2>/dev/null || \
ethtool -C "\${NIC}" rx-usecs 0 tx-usecs 0 adaptive-rx off adaptive-tx off 2>/dev/null || \
ethtool -C "\${NIC}" rx-usecs 0 tx-usecs 0 2>/dev/null || true
ip link set "\${NIC}" mtu "${EFFECTIVE_MTU}" 2>/dev/null || true
ip link set "\${NIC}" txqueuelen 128
...
[Unit]
Description=NIC Tuning — ${NIC} virtio_net ULL
...
_step8() {
  ...
  for DEV in sda sdb vda vdb xvda nvme0n1 nvme1n1; do
    ...
    echo 16 > "/sys/block/${DEV}/queue/nr_requests"   2>/dev/null || true
...
hdr "STEP 12 — IRQBALANCE (pin to CPU0)"
_step12() {
...
_step13() {
  ...
  local sockopt
  sockopt='{"acceptProxyProtocol":false,"tcpFastOpen":false,"mark":0,"tproxy":"off","tcpMptcp":false,"penetrate":false,"domainStrategy":"AsIs","tcpMaxSeg":1440,"dialerProxy":"","tcpKeepAliveInterval":10,"tcpKeepAliveIdle":10,"tcpUserTimeout":20000,"tcpcongestion":"bbr","V6Only":false,"tcpWindowClamp":0,"interface":"","trustedXForwardedFor":[],"addressPortStrategy":"none","customSockopt":[],"tcpNoDelay":true}'
...
hdr "STEP 14 — MEMORY LOCK x-ui HEAP"
_step14() {
  local xui_pid
  xui_pid=$(pgrep -x x-ui 2>/dev/null | head -1 || echo "")
  if [ -n "$xui_pid" ]; then
    chrt -f -p 0 "$xui_pid" 2>/dev/null || true
    ionice -c 1 -n 0 -p "$xui_pid" 2>/dev/null || true
    info "x-ui pid=${xui_pid}: ionice realtime class 1"
  else
...
echo -e "  ${BLD}BDP @150Mbps 30ms    :${RST} ${BDP} bytes (563KB/conn)"
echo -e "  ──────────────────────────────────────────────────────────"
echo -e "  ${BLD}rmem/wmem default    :${RST} $((RMEM_DEFAULT/1024))KB (1xBDP)"
echo -e "  ${BLD}rmem/wmem max        :${RST} $((RMEM_MAX/1024/1024))MB"
echo -e "  ${BLD}notsent_lowat        :${RST} ${NOTSENT_LOWAT} bytes (4KB — ส่งแทบทุก syscall)"
echo -e "  ${BLD}limit_output         :${RST} ${LIMIT_OUTPUT} bytes (1xBDP — anti-bufferbloat)"
echo -e "  ${BLD}fq quantum           :${RST} ${TC_QUANTUM} bytes (1xMSS)"
echo -e "  ${BLD}fq target/interval   :${RST} 1ms / 10ms"
echo -e "  ${BLD}txqueuelen           :${RST} 128"
echo -e "  ${BLD}coalescing           :${RST} rx-usecs=0 tx-usecs=0 rx-frames=1 tx-frames=1"
echo -e "  ${BLD}busy_poll/read       :${RST} ${BUSY_POLL}us"
echo -e "  ${BLD}netdev_budget        :${RST} 600 pkts / 4000us"
echo -e "  ${BLD}NIC IRQ              :${RST} pinned CPU0"
echo -e "  ──────────────────────────────────────────────────────────"
echo -e "  ${BLD}GOMAXPROCS           :${RST} ${VCPU}"
echo -e "  ${BLD}GOGC                 :${RST} 25 (GC บ่อย heap เล็ก pause สั้น)"
echo -e "  ${BLD}GOMEMLIMIT           :${RST} 1200MiB"
echo -e "  ${BLD}GODEBUG              :${RST} madvdontneed=1 asyncpreemptoff=0"
echo -e "  ${BLD}Nice x-ui            :${RST} -15"
...
echo -e "  ──────────────────────────────────────────────────────────"
echo -e "  ${BLD}sched_latency_ns     :${RST} 1ms"
echo -e "  ${BLD}sched_min_gran_ns    :${RST} 0.1ms"
echo -e "  ${BLD}sched_wakeup_ns      :${RST} 0.025ms"
echo -e "  ${BLD}sched_nr_migrate     :${RST} 1"
echo -e "  ${BLD}sched_rt_runtime     :${RST} -1 (no RT throttle)"
echo -e "  ${BLD}timer_migration      :${RST} 0"
echo -e "  ${BLD}nmi_watchdog         :${RST} 0"
echo -e "  ${BLD}watchdog             :${RST} 0"
echo -e "  ${BLD}C-states             :${RST} disabled (pm_qos latency=0)"
echo -e "  ${BLD}THP khugepaged       :${RST} scan_sleep=0 alloc_sleep=0"
echo -e "  ${BLD}dirty_writeback      :${RST} ratio=3%/1% expire=100cs wb=25cs"
echo -e "  ${BLD}vm.stat_interval     :${RST} 10"
echo -e "  ${BLD}fs.pipe-max-size     :${RST} 64KB (ลด pipe buffer = ลด latency)"
echo -e "  ${BLD}tcp_frto             :${RST} 2"
...
echo -e "  tcpKeepAliveIdle : 10"
echo -e "  tcpUserTimeout   : 20000"
