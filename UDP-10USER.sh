...
FLOWS_PER_USER="${FLOWS_PER_USER:-16}"
TCP_SAFE_BUDGET_MB="${TCP_SAFE_BUDGET_MB:-512}"
SWAP_GB="${SWAP_GB:-2}"
UDPGW_PORT="${UDPGW_PORT:-7300}"

PER_USER_DOWN_MBIT="${PER_USER_DOWN_MBIT:-30}"
PER_USER_UP_MBIT="${PER_USER_UP_MBIT:-30}"
...
echo "IFACE=${IFACE} | DNS=${DNS_LABEL} | MTU=${TUNNEL_MTU} | Pipe: Down=${DOWNLOAD_SHAPE_MBIT}mbit Up=${UPLOAD_SHAPE_MBIT}mbit (หาร ${MAX_USERS} user เท่าๆกัน) | Per-user: ${PER_USER_DOWN_MBIT}/${PER_USER_UP_MBIT} Mbps | RTT=${RTT_MS}ms | Max Users=${MAX_USERS}"
...
echo ""
echo "=================================================="
echo -e "${GREEN}ติดตั้ง/ปรับจูนระบบเรียบร้อย (Max ${MAX_USERS} Users | หาร pipe ${DOWNLOAD_SHAPE_MBIT}↓/${UPLOAD_SHAPE_MBIT}↑ Mbit เท่าๆกันคนละ ${PER_USER_DOWN_MBIT}↓/${PER_USER_UP_MBIT}↑ Mbps | UDPGW พอร์ต ${UDPGW_PORT})${NC}"
echo "=================================================="
