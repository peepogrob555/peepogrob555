...
run_step() {
    ...
    if (set -euo pipefail; "$@"); then
        ...
        local rc=$?
        echo -e "\n${RED}${BLD}✘  FAILED: ${name} (exit ${rc})${RST}"
...
echo ""
echo -e "${BLD}${CYN}╔══════════════════════════════════════════════════╗${RST}"
echo -e "${BLD}${CYN}║   VPS SETUP REMASTER v2.1 — VMESS/WS RTT 40ms  ║${RST}"
echo -e "${BLD}${CYN}║   NIC: ${NIC}$(printf '%*s' $((39-${#NIC})) '')║${RST}"
echo -e "${BLD}${CYN}╚══════════════════════════════════════════════════╝${RST}"
echo ""
...
EOF
    ...
    tc qdisc add dev "$NIC" root handle 1: fq \
        quantum 1514 flow_limit 200 buckets 8192 || \
        warn "tc fq บน ${NIC} fail"
}
...
_step6() {
    ...
    ethtool -K "$NIC" gro off lro off tso on gso on 2>/dev/null || \
        warn "ethtool offload บน ${NIC} บางอย่าง fail"
...
echo ""
echo -e "\${BLD}\${CYN}╔══════════════════════════════════════════════╗\${RST}"
echo -e "\${BLD}\${CYN}║   VPS POST-BOOT VERIFY v2.1 — RTT 40ms      ║\${RST}"
echo -e "\${BLD}\${CYN}╚══════════════════════════════════════════════╝\${RST}"
echo ""
...
for p in 22 80 443 2053 2083 2087 2096 8080 8443 54321; do
...
if curl -sf --max-time 5 "http://localhost:2053" &>/dev/null || \
   curl -sf --max-time 5 "http://localhost:2053/login" &>/dev/null; then
    ok "Panel HTTP responding"
else
...
tc qdisc show dev "\$NIC" 2>/dev/null | grep -q "quantum 1514" \
    && ok "tc fq quantum 1514 OK" || wr "tc fq quantum ไม่ถูก"
chk_sysctl net.core.rmem_max                ge 16777216 "rmem_max"
chk_sysctl net.core.wmem_max                ge 16777216 "wmem_max"
chk_sysctl net.ipv4.tcp_notsent_lowat       le 16384    "notsent_lowat"
chk_sysctl net.ipv4.tcp_limit_output_bytes  le 2097152  "limit_output_bytes"
chk_sysctl net.ipv4.tcp_base_mss            eq 1440     "tcp_base_mss"
chk_sysctl net.ipv4.tcp_ecn                 eq 0        "ECN disabled"
chk_sysctl net.ipv4.tcp_fastopen            eq 0        "TFO disabled"
chk_sysctl vm.swappiness                    le 10       "swappiness"
chk_sysctl vm.min_free_kbytes               ge 131072   "min_free_kbytes"
...
ss -tlnp 2>/dev/null | grep -q ':2053 ' \
    && ok "Port 2053 listening" || wr "Port 2053 ไม่ได้ฟัง"
