...
echo -e "  Target: 400Mbps max/conn | RTT 20ms | MTU VPN 1500 | ping low | jitter low\n"
...
sec "STEP 5 — SYSCTL (400Mbps/conn × multi-user | RTT 20ms | low ping | low jitter)"
...
# ── Buffers ───────────────────────────────────────────────────
# BDP per conn = 400Mbps × 20ms = 1MB
# default เล็กไว้ ไม่ pre-alloc ใหญ่ทุก conn → ประหยัด RAM
# max = 16MB รองรับ conn ที่ต้องการ full 400Mbps
...
net.ipv4.tcp_mem = 65536 524288 268435456
...
# 1gbit + besteffort + fair queue = CAKE แบ่งให้แต่ละ flow เท่าๆกัน
# แต่ละ conn ได้สูงสุด 400Mbps ถ้า link ว่าง
...
sec "STEP 8 — VMESS WS SOCKOPT PATCHER (MTU 1500 compensated)"
...
# MTU VPN 1500 (V2BOX locked)
# 1500 - IP(20) - TCP(20) - WS(50) - VMESS(20) = 1390 → ใช้ 1300 เผื่อ safe
SOCKOPT = {
    "tcpNoDelay":           True,
    "tcpKeepAliveIdle":     10,
...
echo -e "${Y}  ▸ TCP Max Seg     : 1300 (auto-patched — ชดเชย MTU VPN 1500)${N}"
echo -e "${Y}  ▸ MTU VPN V2BOX   : 1500 (locked — ชดเชยด้วย TCP Max Seg แล้ว)${N}"
echo -e "${Y}  ▸ max speed/conn  : 400Mbps${N}"
echo -e "${Y}  ▸ CAKE            : 1gbit fair queue ทุก flow${N}"
echo -e "${Y}  ▸ RAM kernel buf  : default 64KB/conn, max 16MB/conn${N}"
