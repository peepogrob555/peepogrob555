...
# สำหรับสตรีมวิดีโอ ซึ่งเปิดอยู่แล้วเพราะ default outgoing = allow และ
# ไม่มี blanket UDP block อยู่ในสคริปต์ชุดนี้ (ถอดออกไปแล้วก่อนหน้านี้
# เพราะขัดกับการเล่นเกม/ดูหนัง — ดู step 4.2c เดิมที่ comment ไว้)

# ── พอร์ตอันตรายที่ปิดอย่างเจาะจง (แม้ default policy จะ deny อยู่แล้ว
#    ก็ระบุไว้ตรงๆ เป็น defense-in-depth กันกรณี policy ถูกเปลี่ยนทีหลัง
...
# ปิด UDP โดยรวม (ค่าเริ่มต้น — ไม่ใช่ blanket block แบบ TCP-only เพราะ
# ขัดกับการเล่นเกม/สตรีมที่ใช้ UDP จริง ดู note step 4.2c) แต่ deny ขาเข้า
# UDP ของพอร์ตที่ไม่จำเป็นไว้เป็น baseline เดิม
ufw deny proto udp from any to any comment "Block inbound UDP (ขาออกยังใช้ได้ปกติสำหรับเกม/สตรีม)"
...
ufw --force enable
ufw status verbose
ok "UFW Firewall เปิดแล้ว — allow: 22/80/443/${PANEL_PORT} + Cloudflare ports | deny: รายชื่อพอร์ตอันตรายข้างบน"
...
PANEL_PORT="${VLESS_PANEL_PORT:-2053}"
TLS_SNI="${VLESS_SNI:-speedtest.net}"
TLS_FINGERPRINT="${VLESS_FP:-firefox}"
EXPECT_RAM_MB=2048
EXPECT_SWAP_MB=4096
...
echo -e "    ${GRN}✔${RST}  UFW allow : 22, 80, 443, ${PANEL_PORT}, + Cloudflare ports (2052/2082/2086/2095/8080/8880/2083/2087/2096/8443)"
echo -e "    ${GRN}✔${RST}  UFW deny  : 21,23,25,110,135,137-139,143,445,1433,3306,3389,5432,5900,6379,8291,11211"
echo -e "  ──────────────────────────────────────────────────────────"
