#!/usr/bin/env bash
# VPS SETUP — VMESS/WS | 1vCPU 2.69GHz | RAM 2GB | eth0 | Ubuntu 22.04
# RTT target: 50ms | BDP = 500Mbps/8 × 0.050 = 3,125,000 bytes
# notsent_lowat = BDP/4 = ~32768
# tcp_limit_output_bytes = BDP × 2 = ~6,291,456 (เผื่อ burst 2 users)
# rmem/wmem max = 32MB (10× BDP headroom)
set -euo pipefail
export LANG=C
...
echo ""
echo -e "${BLD}${CYN}╔══════════════════════════════════════════════════╗${RST}"
echo -e "${BLD}${CYN}║  VPS SETUP — VMESS/WS RTT50ms | Ubuntu 22.04   ║${RST}"
echo -e "${BLD}${CYN}╚══════════════════════════════════════════════════╝${RST}"
echo ""
...
hdr "STEP 5 — KERNEL TCP TUNE (RTT 50ms)"
_step5() {
...
net.ipv4.tcp_limit_output_bytes = 6291456
...
echo ""
echo -e "${BLD}${CYN}╔══════════════════════════════════════════╗${RST}"
echo -e "${BLD}${CYN}║     VPS POST-BOOT VERIFY  (RTT 50ms)    ║${RST}"
echo -e "${BLD}${CYN}╚══════════════════════════════════════════╝${RST}"
echo ""
...
if [ "$(sysctl -n net.core.rmem_max)" -ge 33554432 ];           then ok "rmem_max OK (32MB)";  else bad "rmem_max ต่ำเกิน";      fi
if [ "$(sysctl -n net.core.wmem_max)" -ge 33554432 ];           then ok "wmem_max OK (32MB)";  else bad "wmem_max ต่ำเกิน";      fi
if [ "$(sysctl -n net.ipv4.tcp_notsent_lowat)" -le 65536 ];     then ok "notsent_lowat OK";    else wr  "notsent_lowat สูงเกิน"; fi
if [ "$(sysctl -n net.ipv4.tcp_limit_output_bytes)" -le 8388608 ]; then ok "limit_output_bytes OK"; else wr "limit_output_bytes สูงเกิน"; fi
