...
SWAP_GB="${SWAP_GB:-4}"
PER_USER_TARGET_MBIT="${PER_USER_TARGET_MBIT:-30}"
...
if [ ! -f "$UDP_CUSTOM_CONFIG" ]; then
  echo -e "${RED}ไม่พบ ${UDP_CUSTOM_CONFIG} — ต้องรัน install.sh ของ http-custom-udp-custom ให้เสร็จก่อน${NC}"
  exit 1
fi
...
echo "IFACE=${IFACE} | DNS=${DNS_LABEL} | MTU=${TUNNEL_MTU} | เพดานรวม=${SHAPE_MBIT}mbit | RTT=${RTT_MS}ms | MAX_USERS=${MAX_USERS} | ต่อคน=${PER_USER_TARGET_MBIT}mbit"
...
MAX_USERS=${MAX_USERS}
PER_USER_TARGET_MBIT=${PER_USER_TARGET_MBIT}
DNS_LABEL=${DNS_LABEL}
DNS_V4_A=${DNS_V4_A}
DNS_V4_B=${DNS_V4_B}
DNS_V6_A=${DNS_V6_A}
...
[Unit]
Description=CAKE fair-share QoS
After=network-online.target
Wants=network-online.target
...
[Unit]
Description=Check /var/log flood protection
