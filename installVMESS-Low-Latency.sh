...
echo -e "\n${B}${C}  VPS Setup — VMESS WS | AIS LTE | LOW LATENCY${N}"
...
sec "STEP 5 — SYSCTL KERNEL TUNING (LOW LATENCY)"
...
# ── Buffers — พอดีสำหรับ latency ต่ำ ไม่ over-buffer ──────
...
# ── Keepalive — จับ stall connection เร็ว ──────────────────
net.ipv4.tcp_keepalive_time = 10
net.ipv4.tcp_keepalive_intvl = 3
net.ipv4.tcp_keepalive_probes = 4

# ── Queue — เล็กพอ latency ต่ำ ───────────────────────────
...
# ── ECN — ช่วย congestion signal เร็วขึ้น ─────────────────
net.ipv4.tcp_ecn = 1
...
sec "STEP 7 — CAKE QDISC (RTT=15ms, LOW LATENCY)"

ip link set dev "$ETH" txqueuelen 512 2>/dev/null && \
    ok "txqueuelen -> 512 (low latency)" || inf "txqueuelen skipped"
...
if lsmod | grep -q sch_cake; then
    tc qdisc add dev "$ETH" root cake bandwidth 1gbit rtt 15ms besteffort split-gso 2>/dev/null && \
        ok "CAKE applied: 1gbit rtt 15ms besteffort split-gso" || {
        tc qdisc add dev "$ETH" root cake bandwidth 1gbit rtt 15ms besteffort 2>/dev/null && \
            ok "CAKE applied: 1gbit rtt 15ms besteffort" || {
            tc qdisc add dev "$ETH" root fq_codel target 2ms interval 15ms 2>/dev/null && \
...
#!/bin/bash
ETH=$(ip -o -4 route show to default | awk '{print $5}' | head -1)
ip link set dev $ETH txqueuelen 512 2>/dev/null || true
tc qdisc del dev $ETH root 2>/dev/null || true
...
sec "STEP 8 — VMESS WS SOCKOPT PATCHER (LOW LATENCY)"
...
echo ""
echo -e "${G}${B}╔══════════════════════════════════════════╗${N}"
echo -e "${G}${B}║  DONE — Low Latency Setup complete!      ║${N}"
echo -e "${G}${B}╚══════════════════════════════════════════╝${N}"
