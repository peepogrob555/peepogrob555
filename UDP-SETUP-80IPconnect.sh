...
TCP_RMEM_DEFAULT=$(( PIPE_BYTES / 16 ))
TCP_RMEM_MAX=$PIPE_BYTES
TCP_WMEM_DEFAULT=$(( PIPE_BYTES / 16 ))
TCP_WMEM_MAX=$PIPE_BYTES

UDP_RMEM_MIN=262144
UDP_WMEM_MIN=262144
UDP_MEM_MIN_BYTES=$(( PIPE_BYTES / 8 ))
UDP_MEM_PRESSURE_BYTES=$(( PIPE_BYTES * 3 / 4 ))
UDP_MEM_MAX_BYTES=$PIPE_BYTES
...
echo "SPEC: RAM=${TOTAL_RAM_MB}MB vCPU=${NCPU} | IFACE=${IFACE} | MTU=${TUNNEL_MTU} | Pipe Down/Up=${DOWNLOAD_SHAPE_MBIT}/${UPLOAD_SHAPE_MBIT} Mbps (${PIPE_BYTES} B/s) | MSS=${TCP_MSS_V4}"
...
tc class add dev "$IFACE" parent 1: classid 1:10 htb rate "${DOWNLOAD_SHAPE_MBIT}mbit" ceil "${DOWNLOAD_SHAPE_MBIT}mbit"
...
if ip link show ifb0 >/dev/null 2>&1; then
  ...
  tc class add dev ifb0 parent 1: classid 1:10 htb rate "${UPLOAD_SHAPE_MBIT}mbit" ceil "${UPLOAD_SHAPE_MBIT}mbit"
...
[Unit]
Description=Fixed-rate HTB shaping with fq_codel AQM
...
echo ""
echo "=================================================="
echo -e "${GREEN}ปรับแต่งระบบสำเร็จ | Pipe: ${DOWNLOAD_SHAPE_MBIT}Mbit↓ / ${UPLOAD_SHAPE_MBIT}Mbit↑ (${PIPE_BYTES} B/s, HTB+fq_codel) | BBR+fq_codel | MSS(v4)=${TCP_MSS_V4} | IPv6=DISABLED${NC}"
echo -e "${YELLOW}หมายเหตุ: ipv6.disable=1 ใน GRUB จะสมบูรณ์ 100% หลัง reboot เครื่องหนึ่งครั้ง${NC}"
echo "=================================================="
