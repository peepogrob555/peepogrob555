...
if [[ "${1:-}" == "--rollback" ]]; then
    ...
    # Hiddify rollback — stop docker containers
    docker compose -f /opt/hiddify-manager/docker-compose.yml down 2>/dev/null || true
    ok "Hiddify stopped"
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 4 — ติดตั้ง Hiddify Manager
# ══════════════════════════════════════════════════════════════════════════════

step "Install Hiddify Manager"

if "$DRY_RUN"; then
    info "[DRY] bash <(curl -Ls https://raw.githubusercontent.com/hiddify/hiddify-manager/main/common/download_install.sh)"
else
    echo ""
    _rule "─" "$BCYN"
    printf "  ${BCYN}${BOLD}ติดตั้ง Hiddify Manager${RST}\n"
    printf "  ${DIM}installer จะตั้งค่า panel อัตโนมัติ — ไม่ต้องกรอกอะไร${RST}\n"
    _rule "─" "$BCYN"
    echo ""
...
ok "Hiddify Manager installed"

# ══════════════════════════════════════════════════════════════════════════════
# STEP 5 — ตรวจสอบ Hiddify service
# ══════════════════════════════════════════════════════════════════════════════

step "Verify Hiddify service"

if ! "$DRY_RUN"; then
    sleep 3
    if systemctl is-active hiddify-manager &>/dev/null || \
       systemctl is-active hiddify &>/dev/null || \
       docker ps 2>/dev/null | grep -q hiddify; then
        ok "Hiddify service running"
    else
        warn "Hiddify service not detected — ตรวจสอบด้วย: docker ps หรือ systemctl status hiddify"
    fi
    # แสดง URL panel
    echo ""
    printf "  ${BCYN}${BOLD}Hiddify Panel URL:${RST}\n"
    printf "  ${BWHT}https://%s/  ${DIM}(ดู admin path จาก installer output ข้างบน)${RST}\n" "${PUBLIC_IP:-<IP>}"
    echo ""
else
    info "[DRY] Hiddify service check skipped"
fi
...
if ! "$DRY_RUN"; then
    ...
    _check "Hiddify service active"            "docker ps 2>/dev/null | grep -q hiddify || systemctl is-active hiddify | grep -q active"
...
printf "  ${DIM}%-22s${RST}  ${BWHT}nftables${RST}  ${DIM}SSH:%s · 80 · 443 · %s${RST}\n" \
...
_rule "─" "$DIM"
echo ""
printf "  ${BCYN}${BOLD}HIDDIFY PANEL${RST}\n"
if [[ -n "${XUI_DOMAIN:-}" ]]; then
    printf "  ${BWHT}https://%s:%s/<path>  ${DIM}(ดู webBasePath จาก installer output ข้างบน)${RST}\n"         "$XUI_DOMAIN" "$XUI_PORT"
else
    printf "  ${BWHT}https://%s:%s/<path>  ${DIM}(ดู webBasePath จาก installer output ข้างบน)${RST}\n"         "${PUBLIC_IP}" "$XUI_PORT"
fi
printf "  ${DIM}เข้า panel → สร้าง VLESS Reality inbound → scan QR ด้วย Hiddify App${RST}\n"
echo ""
_rule "─" "$DIM"
echo ""
